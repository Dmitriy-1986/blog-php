...
class to_db2 {
    ...
    public function five_authors ($db) {
					...
					ORDER BY count(`id_author`) DESC LIMIT 3;");
        ...
        public function most_read ($db) {
                 $result4 = $db->query("SELECT * FROM `articles` ORDER BY  `stat` DESC LIMIT 3");
                ...
                $art2['readmore'] = '<br> Читать далее>>';
                return $art2;
        }

        public function comments ($db){
           
                $result_comm = $db->query("SELECT * FROM `comments` ORDER BY `id` DESC LIMIT 3");
